...
def save_psys_keys(ofile, keys1, keys2):
    ofile.write("%s %s   %s %s\n"%(swytrailzro(keys1[0]), swytrailzro(keys1[1]), swytrailzro(keys2[0]), swytrailzro(keys2[1])))
...
def save_particle_systems():
  ...
  for psys in particle_systems:
    ...
    ofile.write("%d %s %s %s %s %s \n"%(psys[num_particles_pos], swytrailzro(psys[life_pos]), swytrailzro(psys[damping_pos]), swytrailzro(psys[gravity_pos]), swytrailzro(psys[turb_size_pos]), swytrailzro(psys[turb_wt_pos])))
    ...
    ofile.write("%s %s %s   "%(swytrailzro(psys[emit_box_size_pos][0]),swytrailzro(psys[emit_box_size_pos][1]),swytrailzro(psys[emit_box_size_pos][2])))
    ofile.write("%s %s %s   "%(swytrailzro(psys[emit_velocity_pos][0]),swytrailzro(psys[emit_velocity_pos][1]),swytrailzro(psys[emit_velocity_pos][2])))
    ofile.write("%s \n"%(swytrailzro(psys[emit_rndmness_pos])))
